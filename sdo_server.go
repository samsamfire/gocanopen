...
func (server *SDOServer) Handle(frame can.Frame) {
	...
	if frame.Data[0] == 0x80 {
		...
		// Copy data and set new new flag
		copy(server.Response.raw[:], frame.Data[:])
		server.RxNew = true
	}
}
...
func (server *SDOServer) Init(od *ObjectDictionary, entry12xx *Entry, nodeId uint8, timeoutTimeMs uint16, canmodule *CANModule) error {
	...
	return server.InitRxTx(server.CANModule, uint16(server.idRxBuff), uint16(server.idTxBuff), server.CobIdClientToServer, server.CobIdServerToClient)
...
func (server *SDOServer) WriteToObjectDictionnry(abortCode *SDOAbortCode, crcOperation uint, crcClient uint16) bool {
	...
	if server.Finished {
		...
		// For string tolerate termination characters (which means length of download could be less)
		varSize := len(server.Streamer.Stream.Data)
		if server.Streamer.Stream.Attribute&ODA_STR != 0 &&
			(varSize == 0 || int(server.SizeTransferred) < varSize) &&
			int(server.BufferOffsetWrite+2) <= len(server.Buffer) {

			server.BufferOffsetWrite++
			server.Buffer[server.BufferOffsetWrite] = 0
			if varSize == 0 || server.SizeTransferred < uint32(varSize) {
				server.BufferOffsetWrite++
				server.Buffer[server.BufferOffsetWrite] = 0
				server.SizeTransferred++

			}
			// TODO should we update datalength ?
			//server.Streamer.Stream.DataOffset
		} else if varSize == 0 {
			// TODO should we update datalength ?
		} else if int(server.SizeTransferred) != varSize {
			if int(server.SizeTransferred) > varSize {
				*abortCode = CO_SDO_AB_DATA_LONG
				server.State = CO_SDO_ST_ABORT
				return false
			} else if int(server.SizeTransferred) < varSize {
...
func (server *SDOServer) ReadFromObjectDictionary(abortCode *SDOAbortCode, countMinimum uint32, calculateCRC bool) bool {
	...
	if !server.Finished && remainingCount < countMinimum {
		...
		// TODO check how to read/write string ?
		// If string, send only up to null end
		// if countRd > 0 && (server.Streamer.Stream.Attribute&ODA_STR) != 0 {
		// 	server.Buffer[remainingCount + uint32(countRd)] = 0
		// 	countStr :=
		// }
...
func (server *SDOServer) Process(nmtIsPreOrOperationnal bool, timeDifferenceUs uint32, timerNextUs *uint32) SDOReturn {
	...
	if server.Valid && server.State == CO_SDO_ST_IDLE && !isNew {
		...
		if server.State == CO_SDO_ST_IDLE {
			...
			if abortCode == CO_SDO_AB_NONE {
				...
				err := server.OD.Find(index).Sub(subIndex, false, server.Streamer)
				if err != nil {
					abortCode = err.(ODR).GetSDOAbordCode()
					server.State = CO_SDO_ST_ABORT
				}
			...
			if upload && abortCode == CO_SDO_AB_NONE {
				...
				server.Finished = false
				if server.ReadFromObjectDictionary(&abortCode, 7, false) {
		...
		if server.State != CO_SDO_ST_IDLE && server.State != CO_SDO_ST_ABORT {
			...
			case CO_SDO_ST_DOWNLOAD_INITIATE_REQ:
				if (response.raw[0] & 0x02) != 0 {
					...
					var countWritten uint16 = 0
					err := server.Streamer.Write(server.Streamer.Stream, buf, &countWritten)
					if err != nil {
						abortCode = err.(ODR).GetSDOAbordCode()
						server.State = CO_SDO_ST_ABORT
						break
				...
				} else if (response.raw[0] & 0x01) != 0 {
					// Segemented transfer check if size indicated
			...
			case CO_SDO_ST_DOWNLOAD_SEGMENT_REQ:
				if (response.raw[0] & 0xE0) == 0x00 {
					...
					if server.Finished || (len(server.Buffer)-int(server.BufferOffsetWrite) < (7 + 2)) {
						if !server.WriteToObjectDictionnry(&abortCode, 0, 0) {
							break
						}
					}
			...
			case CO_SDO_ST_DOWNLOAD_BLK_END_REQ:
				...
				if !server.WriteToObjectDictionnry(&abortCode, 2, crcClient) {
					break
				}
			...
			case CO_SDO_ST_UPLOAD_BLK_SUBBLOCK_SREQ, CO_SDO_ST_UPLOAD_BLK_SUBBLOCK_CRSP:
				...
				if !server.ReadFromObjectDictionary(&abortCode, uint32(server.BlockSize)*7, true) {
					break
				}
		...
		case CO_SDO_ST_DOWNLOAD_BLK_SUBBLOCK_RSP:
			...
			if server.Finished {
				...
				if count > 127 {
					...
					// Empty buffer
					if !server.WriteToObjectDictionnry(&abortCode, 1, 0) {
						break
					}
...
func (server *SDOServer) Abort(abortCode SDOAbortCode) {
	...
	server.CANModule.Send(*server.CANtxBuff)
	log.Warnf("Aborting on server side (x%x): %v", uint32(abortCode), abortCode)
