...
func NewGatewayServer(network *network.Network, defaultNetworkId uint16, defaultNodeId uint8, sdoUploadBufferSize int) *GatewayServer {
	...
	// CiA 309-5 | 4.3
	gw.addRoute("start", createNmtHandler(base, nmt.NMT_ENTER_OPERATIONAL))
	gw.addRoute("stop", createNmtHandler(base, nmt.NMT_ENTER_STOPPED))
	gw.addRoute("preop", createNmtHandler(base, nmt.NMT_ENTER_PRE_OPERATIONAL))
	gw.addRoute("preoperational", createNmtHandler(base, nmt.NMT_ENTER_PRE_OPERATIONAL))
	gw.addRoute("reset/node", createNmtHandler(base, nmt.NMT_RESET_NODE))
	gw.addRoute("reset/comm", createNmtHandler(base, nmt.NMT_RESET_COMMUNICATION))
	gw.addRoute("reset/communication", createNmtHandler(base, nmt.NMT_RESET_COMMUNICATION))
