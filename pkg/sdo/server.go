...
func (server *SDOServer) writeObjectDictionary(crcOperation uint, crcClient crc.CRC16) error {
	...
	if server.finished {
		...
		if server.streamer.CheckHasAttribute(od.ATTRIBUTE_STR) &&
			...
			server.streamer.SetDataLength(server.sizeTransferred)
	...
	if !server.finished && buffered < countMinimum {
		...
		if countRd > 0 && server.streamer.CheckHasAttribute(od.ATTRIBUTE_STR) {
			...
			if countStr < countRd {
				...
				countRd = countStr
				err = nil
				server.streamer.SetDataLength(server.sizeTransferred + uint32(countRd))
			}
		}
			...
			case SDO_STATE_DOWNLOAD_INITIATE_REQ:
				if (response.raw[0] & 0x02) != 0 {
					...
					if server.streamer.CheckHasAttribute(od.ATTRIBUTE_STR) &&
						...
						server.streamer.SetDataLength(uint32(dataSizeToWrite))
					} else if varSizeInOd == 0 {
						server.streamer.SetDataLength(uint32(dataSizeToWrite))
...
func NewSDOServer(
	...
	if entry12xx == nil {
		...
		canIdClientToServer = CLIENT_ID + uint16(nodeId)
		canIdServerToClient = SERVER_ID + uint16(nodeId)
		server.valid = true
	} else {
